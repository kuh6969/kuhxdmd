...
if (anu.action == 'add') {
xxyneo.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `Hallo @${num.split("@")[0]} Selamat Datang Di Grup *${metadata.subject}*\n\nSemoga Kamu Senang Berada Disini Serta Jangan Lupa Untuk Membaca Dan Mematuhi Rules Yang Ada` })
} else if (anu.action == 'remove') {
xxyneo.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `Goodbye @${num.split("@")[0]}\n\nTetap Putus Asa Jangan Semangat Dan Jadilah Beban Keluarga 🤙🗿` })
}
}
} catch (err) {
console.log(err)
